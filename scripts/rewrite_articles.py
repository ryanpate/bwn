...
class ArticleRewriter:
    ...
    def _create_backup(self, filepath: Path):
        ...
        backup_file = backup_path.parent / \
    ...
    def _should_rewrite(self, filepath: Path, frontmatter: Dict) -> bool:
        ...
        marker_count = sum(
            1 for marker in millennial_markers if marker in content)
        if marker_count >= 3:
    ...
    def rewrite_article(self, filepath: Path) -> bool:
        ...
        try:
            ...
            frontmatter['last_rewrite'] = datetime.datetime.now(
            ).isoformat() + "Z"
...
def main():
    parser = argparse.ArgumentParser(
        description='Rewrite existing articles with millennial voice')
    ...
    if args.specific:
        ...
        articles = rewriter.get_articles_to_rewrite(
            limit=args.limit, since_date=args.since)
        ...
        print(f"\n{'='*50}")
        print(
            f"✅ Successfully rewrote: {success_count}/{len(articles)} articles")
        if not args.dry_run:
